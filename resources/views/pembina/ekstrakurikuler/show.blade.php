    ...
    <div class="p-8">
        ...
        <div class=" mx-5 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                <form
                    action="{{ route('pembina.ekstrakurikuler.update', ['id' => $dataEkstrakurikuler['id_data_ekstrakurikuler']]) }}"
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <div class="relative">
                                <input type="date" name="tanggal" placeholder="Select date"
                                    value="{{ $dataEkstrakurikuler['tanggal'] }}"
                        ...
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                Ekstrakurikuler<span class="text-error-500">*</span>
                            </label>
                            <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                <select name="id_ekstrakurikuler" required
                                    class="dark:bg-dark-900 shadow-theme-xs  focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border  bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden  dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 {{ $errors->has('id_ekstrakurikuler') ? 'border-error-300 focus:border-error-300 dark:border-error-700 dark:focus:border-error-800' : 'border-gray-300 focus:border-brand-300 dark:border-gray-700' }}">
                                    <option value="">Pilih Ekstrakurikuler</option>
                                    @foreach ($ekstrakurikulers as $ekstrakurikuler)
                                        <option value="{{ $ekstrakurikuler['id_ekstrakurikuler'] }}"
                                            {{ $dataEkstrakurikuler->ekstrakurikuler->id_ekstrakurikuler == $ekstrakurikuler['id_ekstrakurikuler'] ? 'selected' : '' }}>
                                            {{ $ekstrakurikuler['name'] }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="max-w-full overflow-x-auto">
                            <table class="min-w-full">
                                ...
                                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                    @foreach ($dataEkstrakurikuler->presensi->filter(fn($p) => $p->id_santri !== null) as $index => $presensi)
                                        <tr>
                                                ...
                                                <div class="flex items-center">
                                                    <div class="flex -space-x-2 w-full">
                                                        <div x-data="{ isOptionSelected: false }"
                                                            ...
                                                            <select name="status_santri[]" required
                                                                class="w-full min-w-[120px] dark:bg-dark-900 shadow-theme-xs focus:ring-brand-500/10 h-11 appearance-none rounded-lg border bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 {{ $errors->has('status_santri.' . $index) ? 'border-error-300 focus:border-error-300 dark:border-error-700 dark:focus:border-error-800' : 'border-gray-300 focus:border-brand-300 dark:border-gray-700' }}"
                                            ...
                                            <td class="px-5 py-4 sm:px-6  min-w-[140px]">
                                                <div class="flex items-center">
                                                    <div class="flex -space-x-2  w-full">
                                                        <input type="text" name="catatan_santri[]"
                                                            value="{{ $presensi['catatan'] }}"
                                    ...
                                    @endforeach
                                    @foreach ($dataEkstrakurikuler->presensi->filter(fn($p) => $p->id_santriwati !== null) as $index => $presensi)
                                        <tr>
                                            <td class="px-5 py-4 sm:px-6">
                                                <div class="flex items-center">
                                                    <div class="flex items-center gap-3">
                                            ...
                                            <td class="px-5 py-4 sm:px-6 min-w-[100px]">
                                                <div class="flex items-center">
                                                    <div class="flex -space-x-2 w-full">
                                                        <div x-data="{ isOptionSelected: false }"
                                                            class="relative z-20 bg-transparent w-full">
                                                            <select name="status_santriwati[]" required
                                                                class="w-full min-w-[120px] dark:bg-dark-900 shadow-theme-xs focus:ring-brand-500/10 h-11 appearance-none rounded-lg border bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 {{ $errors->has('status_santriwati.' . $index) ? 'border-error-300 focus:border-error-300 dark:border-error-700 dark:focus:border-error-800' : 'border-gray-300 focus:border-brand-300 dark:border-gray-700' }}"
                                                                :class="isOptionSelected && 'text-gray-800 dark:text-white/90'"
                                                                @change="isOptionSelected = true">
                                                                <option
                                                                    {{ $presensi['status'] === 'hadir' ? 'selected' : '' }}
                                            ...
                                            <td class="px-5 py-4 sm:px-6  min-w-[140px]">
                                                <div class="flex items-center">
                                                    <div class="flex -space-x-2  w-full">
                                                        <input type="text" name="catatan_santriwati[]"
                                                            value="{{ $presensi['catatan'] }}"
                                                            class="dark:bg-dark-900 shadow-theme-xs  focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border  bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden  dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 border-gray-300 focus:border-brand-300 dark:border-gray-700" />
                                                    </div>
                                                </div>
