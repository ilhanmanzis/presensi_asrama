        ...
        <div class=" mx-5 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="px-5 py-4 sm:px-6 sm:py-5 flex justify-between">
                <h3 class="text-base font-medium text-gray-800 dark:text-white/90">
                    Anggota Kelompok Sorogan Kitab
                </h3>
            </div>
                ...
                <div x-data="anggotaApp()">
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <button @click="cariSantri" class="bg-blue-500 text-white px-3 py-2 rounded">
                                Cari
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="my-4">
                        ...
                        <template x-if="hasilPencarian.length > 0">
                            <table class="w-full">
                                ...
                                <tbody>
                                    <template x-for="santri in hasilPencarian" :key="santri.id_santri">
                                        <tr>
                                            <td class="px-5 py-4 sm:px-6">
                                                <span x-text="santri.nis"
                                                    class="block font-medium text-gray-800 text-theme-sm dark:text-white/90"></span>
                                            </td>
                                            <td class="px-5 py-4 sm:px-6">
                                                <span x-text="santri.name"
                                                    class="block font-medium text-gray-800 text-theme-sm dark:text-white/90"></span>
                                            </td>
                                            <td class="px-5 py-4 sm:px-6">
                                                <button @click="tambah(santri)" :disabled="sudahAda(santri.id_santri)"
                                                    :class="sudahAda(santri.id_santri) ?
                    ...
                    <form method="POST"
                        :action="'{{ route('admin.santri-kitab.simpan', $kelompok->id_kitab_santri) }}'">
                        @csrf
                ...
                <script>
                    function anggotaApp() {
                        return {
                            ...
                            cariSantri() {
                                fetch(`/admin/cari-santri?q=${this.keyword}`)
                                    .then(res => res.json())
                                    .then(data => {
                            ...
                            tambah(santri) {
                                if (!this.sudahAda(santri.id_santri)) {
                                    this.anggota.push({
                                        id: santri.id_santri,
                                        name: santri.name,
                                        nis: santri.nis
