        ...
        <div class=" mx-5 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="px-5 py-4 sm:px-6 sm:py-5 flex justify-between">
                <h3 class="text-base font-medium text-gray-800 dark:text-white/90">
                    Edit Nama Kelompok Sorogan Al-Quran
                </h3>
            </div>
            <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                <form
                    action="{{ route('admin.santriwati-alquran.update', ['id' => $alquran['id_alquran_santriwati']]) }}"
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ $alquran['name'] }}"
                        ...
                        <div>
                            ...
                            <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                <select name="status" required
                                    ...
                                    <option value="aktif" {{ $alquran->status == 'aktif' ? 'selected' : '' }}>
                                        Aktif</option>
                                    <option value="nonaktif" {{ $alquran->status == 'nonaktif' ? 'selected' : '' }}>
                                        Nonaktif</option>
                                </select>
                                @error('status')
                                    <p class="text-theme-xs text-error-500 my-1.5">
