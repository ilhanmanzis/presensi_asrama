@if (empty($data['dates']) || count($data['dates']) === 0)
    <div class="header">
        <h2>Laporan Presensi Ekstrakurikuler {{ $data['name'] }}</h2>
        ...
        <tbody>
            ...
            @foreach ($data['presensis'] as $index => $peserta)
                <tr>
                    ...
                    <td valign="center" align="left">{{ $peserta['nis'] }}</td>
                    <td valign="center" align="left" class="nama-col">{{ $peserta['nama'] }}</td>
                    @foreach ($data['dates'] as $date)
                        @php
                    ...
                    @endforeach
                    <td valign="center" align="center">{{ $peserta['hadir'] }}</td>
                    <td valign="center" align="center">{{ $peserta['sakit'] }}</td>
                    <td valign="center" align="center">{{ $peserta['izin'] }}</td>
                    <td valign="center" align="center">{{ $peserta['alpha'] }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
